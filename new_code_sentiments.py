...
def create_metrics_comparison_view(df):
    """
    Creates a comprehensive metrics view comparing sentiment changes across different periods
    specifically for Consigner customer type across PlayStore, Trip Feedbacks, and Escalations.
    """
    
    # Filter for Consigner customer type
    consigner_df = df[df['customer_type'] == 'Consigner'].copy()
    
    if consigner_df.empty:
        st.warning("No Consigner data available for the selected period. Please check your filters.")
        return
    
    st.markdown('<h2 class="text-2xl md:text-3xl font-semibold text-slate-800 text-center mb-6">📊 Consigner Sentiment Metrics: Period-over-Period Analysis</h2>', unsafe_allow_html=True)
    st.markdown("<p class='text-slate-600 text-center mb-8'>Track sentiment evolution from March-June vs July for Consigner customers across all touchpoints</p>", unsafe_allow_html=True)
    
    # Ensure timestamp column exists and is datetime
    if 'timestamp' not in consigner_df.columns or consigner_df['timestamp'].isna().all():
        st.error("Timestamp data is required for period comparison. Please ensure your data has valid timestamp information.")
        return
    
    # Create period classifications
    consigner_df['month'] = consigner_df['timestamp'].dt.month
    consigner_df['month_name'] = consigner_df['timestamp'].dt.strftime('%B')
    
    # Define periods
    consigner_df['period'] = consigner_df['month'].apply(
        lambda x: 'Mar-Jun' if x in [3, 4, 5, 6] else 'Jul' if x == 7 else 'Other'
    )
    
    # Filter for only the periods we're interested in
    period_df = consigner_df[consigner_df['period'].isin(['Mar-Jun', 'Jul'])].copy()
    
    if period_df.empty:
        st.warning("No data available for March-June or July periods. Please adjust your date range.")
        return
    ...
    overall_metrics = period_df.groupby('period').agg({
        'feedback': 'count',
        'rating': 'mean',
    ...
    sentiment_dist = period_df.groupby(['period', 'predicted_sentiment']).size().unstack(fill_value=0)
    ...
    with col1:
        st.markdown("#### March-June Period")
        if 'Mar-Jun' in overall_metrics.index:
            mar_jun_data = overall_metrics.loc['Mar-Jun']
            st.metric("Total Feedback", int(mar_jun_data['feedback']))
            st.metric("Average Rating", f"{mar_jun_data['rating']:.1f}/5")
            st.metric("Average Sentiment Score", f"{mar_jun_data['sentiment_score']:.2f}")
            st.metric("Escalations", int(mar_jun_data['escalated_flag']))
            
            if 'Mar-Jun' in sentiment_percentages.index:
                sent_data = sentiment_percentages.loc['Mar-Jun']
                st.write("**Sentiment Distribution:**")
                for sentiment in ['Positive', 'Neutral', 'Negative']:
                    if sentiment in sent_data.index:
                        st.write(f"• {sentiment}: {sent_data[sentiment]:.1f}%")
        else:
            st.info("No data available for March-June period")
    
    with col2:
        st.markdown("#### July Period")
        if 'Jul' in overall_metrics.index:
            jul_data = overall_metrics.loc['Jul']
            st.metric("Total Feedback", int(jul_data['feedback']))
            st.metric("Average Rating", f"{jul_data['rating']:.1f}/5")
            st.metric("Average Sentiment Score", f"{jul_data['sentiment_score']:.2f}")
            st.metric("Escalations", int(jul_data['escalated_flag']))
            
            if 'Jul' in sentiment_percentages.index:
                sent_data = sentiment_percentages.loc['Jul']
                st.write("**Sentiment Distribution:**")
                for sentiment in ['Positive', 'Neutral', 'Negative']:
                    if sentiment in sent_data.index:
                        st.write(f"• {sentiment}: {sent_data[sentiment]:.1f}%")
        else:
            st.info("No data available for July period")
    
    # === CHANGE INDICATORS ===
    if 'Mar-Jun' in overall_metrics.index and 'Jul' in overall_metrics.index:
        ...
        with change_col1:
            feedback_change = jul_data['feedback'] - mar_jun_data['feedback']
            st.metric("Feedback Volume Change", f"{feedback_change:+.0f}", delta=f"{feedback_change:+.0f}")
        
        with change_col2:
            rating_change = jul_data['rating'] - mar_jun_data['rating']
            st.metric("Rating Change", f"{rating_change:+.2f}", delta=f"{rating_change:+.2f}")
        
        with change_col3:
            sentiment_change = jul_data['sentiment_score'] - mar_jun_data['sentiment_score']
            st.metric("Sentiment Score Change", f"{sentiment_change:+.2f}", delta=f"{sentiment_change:+.2f}")
        
        with change_col4:
            escalation_change = jul_data['escalated_flag'] - mar_jun_data['escalated_flag']
            st.metric("Escalation Change", f"{escalation_change:+.0f}", delta=f"{escalation_change:+.0f}")
    
    st.markdown("---")
    
    # === SOURCE-SPECIFIC ANALYSIS ===
    st.markdown("### 📱 Source-Specific Sentiment Analysis")
    
    # Define sources and their display names
    sources = {
        'PlayStore': 'PlayStore Reviews',
        'Trip': 'Trip Feedbacks',
        'Escalation': 'Escalations'
    }
    
    # Create tabs for different sources
    source_tabs = st.tabs(list(sources.values()))
    
    for idx, (source_key, source_name) in enumerate(sources.items()):
        with source_tabs[idx]:
            # Filter data for this source
            source_df = period_df[period_df['source'].str.contains(source_key, case=False, na=False)]
            
            if source_df.empty:
                st.info(f"No {source_name} data available for the selected periods.")
                continue
            
            # Source-specific metrics
            source_metrics = source_df.groupby('period').agg({
                'feedback': 'count',
                'rating': 'mean',
                'sentiment_score': 'mean',
                'escalated_flag': 'sum'
            ...
            st.markdown(f"#### {source_name} Metrics Comparison")
            
            if len(source_metrics) > 0:
                # Create metrics table
                metrics_df = source_metrics.reset_index()
                metrics_df.columns = ['Period', 'Total Feedback', 'Avg Rating', 'Avg Sentiment Score', 'Escalations']
                ...
                with col1:
                    ...
                    source_sentiment_dist = source_df.groupby(['period', 'predicted_sentiment']).size().unstack(fill_value=0)
                    
                    if not source_sentiment_dist.empty:
                        fig_sentiment = px.bar(
                            source_sentiment_dist.reset_index(),
                            x='period',
                            y=['Positive', 'Neutral', 'Negative'],
                            title=f"{source_name} - Sentiment Distribution by Period",
                            color_discrete_map={
                                'Positive': '#00CC96',
                                'Negative': '#EF553B',
                                'Neutral': '#FFA15A'
                ...
                with col2:
                    # Average rating and sentiment score trend
                    if not source_metrics.empty:
                        fig_trends = make_subplots(
                            rows=2, cols=1,
                            subplot_titles=('Average Rating', 'Average Sentiment Score'),
                        ...
                        fig_trends.add_trace(
                            go.Scatter(
                                x=source_metrics.index,
                                y=source_metrics['rating'],
                                mode='lines+markers',
                                name='Rating',
                        ...
                        fig_trends.add_trace(
                            go.Scatter(
                                x=source_metrics.index,
                                y=source_metrics['sentiment_score'],
                                mode='lines+markers',
                        ...
                        fig_trends.update_layout(
                            height=400,
                            title_text=f"{source_name} - Rating & Sentiment Trends",
                            showlegend=False
                        )
                        st.plotly_chart(fig_trends, use_container_width=True)
                
                # Calculate and display changes if both periods exist
                if 'Mar-Jun' in source_metrics.index and 'Jul' in source_metrics.index:
                    st.markdown(f"#### {source_name} - Key Changes (Mar-Jun → Jul)")
                    
                    mar_jun_source = source_metrics.loc['Mar-Jun']
                    jul_source = source_metrics.loc['Jul']
                    
                    change_col1, change_col2, change_col3 = st.columns(3)
                    
                    with change_col1:
                        feedback_change = jul_source['feedback'] - mar_jun_source['feedback']
                        feedback_pct = (feedback_change / mar_jun_source['feedback']) * 100 if mar_jun_source['feedback'] > 0 else 0
                        st.metric(
                            "Volume Change",
                            f"{feedback_change:+.0f}",
                            delta=f"{feedback_pct:+.1f}%"
                        )
                    
                    with change_col2:
                        rating_change = jul_source['rating'] - mar_jun_source['rating']
                        rating_pct = (rating_change / mar_jun_source['rating']) * 100 if mar_jun_source['rating'] > 0 else 0
                        st.metric(
                            "Rating Change",
                            f"{rating_change:+.2f}",
                            delta=f"{rating_pct:+.1f}%"
                        )
                    
                    with change_col3:
                        sentiment_change = jul_source['sentiment_score'] - mar_jun_source['sentiment_score']
                        st.metric(
                            "Sentiment Change",
                            f"{sentiment_change:+.2f}",
                            delta=f"{sentiment_change:+.2f}"
                        )
    ...
    if 'Mar-Jun' in overall_metrics.index and 'Jul' in overall_metrics.index:
        ...
        # Volume insight
        feedback_change = jul_data['feedback'] - mar_jun_data['feedback']
        if feedback_change > 0:
            insights.append(f"📈 **Volume Increase**: Consigner feedback volume increased by {feedback_change:.0f} from Mar-Jun to Jul")
        elif feedback_change < 0:
            insights.append(f"📉 **Volume Decrease**: Consigner feedback volume decreased by {abs(feedback_change):.0f} from Mar-Jun to Jul")
        
        # Rating insight
        rating_change = jul_data['rating'] - mar_jun_data['rating']
        if rating_change > 0.1:
            insights.append(f"⭐ **Rating Improvement**: Average rating improved by {rating_change:.2f} points")
        elif rating_change < -0.1:
            insights.append(f"⚠️ **Rating Decline**: Average rating decreased by {abs(rating_change):.2f} points")
        
        # Sentiment insight
        sentiment_change = jul_data['sentiment_score'] - mar_jun_data['sentiment_score']
        if sentiment_change > 0.1:
            insights.append(f"😊 **Sentiment Improvement**: Overall sentiment score improved by {sentiment_change:.2f}")
        elif sentiment_change < -0.1:
            insights.append(f"😞 **Sentiment Decline**: Overall sentiment score decreased by {abs(sentiment_change):.2f}")
        
        # Escalation insight
        escalation_change = jul_data['escalated_flag'] - mar_jun_data['escalated_flag']
        if escalation_change > 0:
            insights.append(f"🚨 **Escalation Increase**: Escalations increased by {escalation_change:.0f}")
        elif escalation_change < 0:
            insights.append(f"✅ **Escalation Decrease**: Escalations decreased by {abs(escalation_change):.0f}")
    
    if insights:
        for insight in insights:
            st.markdown(insight)
    else:
        st.info("Generate insights by ensuring data is available for both Mar-Jun and Jul periods.")
    ...
    # Prepare export data
    export_data = period_df[['timestamp', 'source', 'customer_type', 'rating', 'feedback', 
                            'predicted_sentiment', 'sentiment_score', 'escalated_flag', 'period']].copy()
    
    csv_output = export_data.to_csv(index=False).encode('utf-8')
    st.download_button(
        label="⬇️ Download Consigner Metrics Data (CSV)",
        data=csv_output,
        file_name=f"consigner_metrics_comparison_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
        mime="text/csv",
        help="Download the filtered Consigner data used for this metrics comparison."
    )
